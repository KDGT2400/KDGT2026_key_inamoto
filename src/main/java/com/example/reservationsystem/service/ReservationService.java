...
public ReservationService(ReservationRepository reservationRepository, UserRepository
userRepository, ShiftRepository shiftRepository) {
//フィールドへ予約リポジトリを設定
...
@Transactional
public Reservation createReservation(User customer, Long staffId, LocalDate date, LocalTime
timeSlot, String menu) {
...
Optional<com.example.reservationsystem.entity.Shift> staffShift =
shiftRepository.findByStaffAndDate(staff, date);
//シフト未設定なら空き枠なし
...
List<LocalTime> allPossibleSlots = generateTimeSlots(shiftStart, shiftEnd, 30); // 30 minuteintervals
